...
public class DemoSQLConfig extends APIJSONSQLConfig<Long> {
    ...
    public String getDBUri() {
        ...
        if (isElasticsearch()) { // Elasticsearch 配置文档  https://www.elastic.co/guide/en/elasticsearch/reference/current/_api_usage.html
            //return "jdbc:es://https://test-d0f0b1.es.us-central1.gcp.cloud.es.io:9243?timezone=UTC&ssl=false&page.size=250&apikey=search-fzrbwe3g4vb4wsjmnrkrwdjm";
            return "jdbc:es://http://localhost:9200/?timezone=UTC&page.size=250&ssl=false"; // TODO 改成你自己的
        }
        if (isPresto()) { // Presto(PrestoDB) 配置文档  https://prestodb.io/docs/current/installation/jdbc.html#connecting
            return "jdbc:presto://localhost:8099/mysql?SSL=false"; // TODO 改成你自己的
        }
        if (isTrino()) { // Trino(PrestoSQL) 配置文档  https://trino.io/docs/current/client/jdbc.html#connecting
            return "jdbc:trino://localhost:8099/mysql?SSL=false"; // TODO 改成你自己的
        }
