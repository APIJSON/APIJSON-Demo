...
public class DemoController { // extends APIJSONController<Long> {
	...
	public String parse(RequestMethod method, String request, SessionState session) {
	...
	@Post
	@Mapping("get")
	public String get(@Body String request, SessionState session) {
	 ...
	 */
	@Post
	@Mapping("{method}")  // 如果和其它的接口 URL 冲突，可以加前缀，例如改为 crud/{method} 或 Controller 注解 @RequestMapping("crud")
//	@Override
	public String crud(@Path String method, @Body String request, SessionState session) {
	...
	@Get
	@Mapping("get/{request}")
	public String openGet(@Path String request, SessionState session) {
		try {
