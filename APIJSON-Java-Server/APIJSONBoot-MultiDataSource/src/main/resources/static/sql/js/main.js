...
// #### 右上角设置项 > 预览请求输入框，显示对应的预览效果`
      ...
      otherEnv: 'jdbc:mysql://localhost:3306',  // 其它环境数据库地址，用来对比当前的
...
//                  'testAccountId': this.getCurrentAccountId(),
      ...
      saveCache: function (url, key, value) {
        ...
        cache[key] = value
        localStorage.setItem('SQLAuto:' + url, JSON.stringify(cache))
      },
...
Content-Type: ` + contentType) + (StringUtil.isEmpty(headerStr, true) ? '' : headerStr)
      ...
      setDoc: function (d) {
        ...
        vOutput.value = (this.isTestCaseShow ? '' : output) + (
          ...
          + '<p align="center">SQLAuto-智能零代码数据库测试工具'
          + '<br>机器学习零代码测试、任意模板变量增删改查、一键批量生成测试数据'
...
//      log('getDoc  callback(doc); = \n' + doc);
      ...
      testRandom: function (show, testList, testSubList, limit, isCross, isManual, callback) {
        ...
        if (testList != true && testSubList != true) {
          ...
          for (var i = 0; i < (limit != null ? limit : list.length); i ++) {  //limit限制子项测试个数
            ...
            try {
              this.testRandomSingle(show, false, itemAllCount > 1 && !testSubList, item, this.type, url, json, {}, isCross, isManual, function (url, res, err) {
                var data = null
       ...
       * @param callback
       */
      testRandomSingle: function (show, testList, testSubList, item, type, url, sql, header, isCross, isManual, callback) {
        item = item || {}
       ...
       * @param callback
       */
      parseRandom: function (sql, config, randomId, generateJSON, generateConfig, generateName, callback, preScript) {
      ...
      compareResponse: function (allCount, list, index, item, response, isRandom, accountIndex, justRecoverTest, err, ignoreTrend, isCross, callback) {
        ...
        if (err != null) {
          ...
          tr.compare = JSONResponse.compareResponse(standard, rsp, '', isML, null, null, ignoreTrend) || {}
    ...
    created: function  () {
      ...
      if (rawReq == null || StringUtil.isEmpty(rawReq.type, true)) {
        ...
        setTimeout(function () {
          ...
          var hasTestArg = false  // 避免 http://localhost:63342/SQLAuto/index.html?_ijt=fh8di51h7qip2d1s3r3bqn73nt 这种无意义参数
